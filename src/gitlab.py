...
def main(wf):
    ...
    parser.add_argument("--setage", dest="maxage", nargs="?", default=None)
    ...
    if args.maxage:
        log.info(
            "Setting project cache maxage to {age} seconds".format(age=args.maxage)
        )
        wf.settings["max_age"] = int(args.maxage)
        return 0
    ...
    query = args.query
    age = wf.settings.get("max_age", 3600)

    projects = wf.cached_data("projects", None, max_age=age)
    ...
    if not wf.cached_data_fresh("projects", max_age=age) and not is_running("update"):
...
if __name__ == u"__main__":
    wf = Workflow3(update_settings={"github_slug": "lukewaite/alfred-gitlab"})
    log = wf.logger
